        ...
        <div className={`grid gap-3 px-8 pt-2 pb-2 text-center text-base ${
          ...
          {transaction.sender && (
            <div className="text-black font-bold text-base py-1">
              ...
              <span className="block text-lg">{transaction.sender}</span>
            </div>
          )}
          <div className="text-black font-bold text-base py-1">
            <span className="font-bold text-gray-700 block mb-1">المستفيد</span>
            <span className="block text-lg break-words leading-relaxed">{transaction.receiver}</span>
          </div>
          {transaction.receiver_mobile && (
            <div className="text-black font-bold text-base py-1">
              <span className="font-bold text-gray-700 block mb-1">الجوال</span>
              <span className="block text-lg">{transaction.receiver_mobile}</span>
            </div>
          )}
        </div>
        {/* المبلغ */}
        <div className="px-8 py-3 text-center bg-gradient-to-l from-yellow-50 to-white border-y-2 border-yellow-300 my-2">
          <div className="text-2xl font-extrabold text-green-700 mb-1">{amount} {transaction.currency === 'USD' ? 'دولار' : transaction.currency}</div>

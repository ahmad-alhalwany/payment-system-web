  ...
  return (
    <form className="max-w-2xl mx-auto space-y-6" onSubmit={handleSubmit}>
      <Modal open={showErrorModal} onClose={() => setShowErrorModal(false)}>
        <div className="text-red-700 text-lg font-bold mb-2">حدث خطأ</div>
        <ul className="list-disc pr-5 text-right">
          {errors.map((err, i) => <li key={i}>{err}</li>)}
        </ul>
        <button
          className="mt-4 bg-primary-500 text-white px-6 py-2 rounded-lg font-bold hover:bg-primary-600 transition"
      ...
      {showConfirm ? (
        ...
        <>
          <h2 className="text-xl font-bold text-primary-700 mb-2">بيانات المرسل</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="font-semibold">اسم المرسل <span className="text-red-500">*</span></label>
              <input className="input" placeholder="اسم المرسل" value={sender.name} onChange={e => setSender({ ...sender, name: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">رقم الهاتف <span className="text-red-500">*</span></label>
              <input className="input" placeholder="رقم الهاتف (10 أرقام)" value={sender.mobile} onChange={e => setSender({ ...sender, mobile: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">رقم الهوية <span className="text-red-500">*</span></label>
              <input className="input" placeholder="رقم الهوية" value={sender.id} onChange={e => setSender({ ...sender, id: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">المحافظة <span className="text-red-500">*</span></label>
              <select className="input" value={sender.governorate} onChange={e => setSender({ ...sender, governorate: e.target.value })} required>
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              ...
              <input className="input" placeholder="العنوان" value={sender.address} onChange={e => setSender({ ...sender, address: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1 md:col-span-2">
              <label className="font-semibold">الموقع</label>
              <input className="input" placeholder="الموقع" value={sender.location} onChange={e => setSender({ ...sender, location: e.target.value })} />
            </div>
          </div>
          <h2 className="text-xl font-bold text-primary-700 mb-2">بيانات المستلم</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="font-semibold">اسم المستلم <span className="text-red-500">*</span></label>
              <input className="input" placeholder="اسم المستلم" value={receiver.name} onChange={e => setReceiver({ ...receiver, name: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">رقم الهاتف <span className="text-red-500">*</span></label>
              <input className="input" placeholder="رقم الهاتف (10 أرقام)" value={receiver.mobile} onChange={e => setReceiver({ ...receiver, mobile: e.target.value })} required />
            </div>
            <div className="flex flex-col gap-1 md:col-span-2">
              <label className="font-semibold">المحافظة <span className="text-red-500">*</span></label>
              <select className="input" value={receiver.governorate} onChange={e => {
          ...
          <h2 className="text-xl font-bold text-primary-700 mb-2">تفاصيل التحويل</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="flex flex-col gap-1">
              <label className="font-semibold">المبلغ <span className="text-red-500">*</span></label>
              <input className="input" placeholder="المبلغ" value={amount} onChange={e => setAmount(e.target.value)} required />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">المبلغ المستفاد <span className="text-gray-400">(اختياري، يطبق عليه الضريبة)</span></label>
              <input className="input" placeholder="المبلغ المستفاد (يطبق عليه الضريبة)" value={benefitAmount} onChange={e => setBenefitAmount(e.target.value)} />
            </div>
            <div className="flex flex-col gap-1">
              <label className="font-semibold">العملة <span className="text-red-500">*</span></label>
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              <label className="font-semibold">الفرع المستلم <span className="text-red-500">*</span></label>
              <select
                className="input"
                value={branch}
            ...
            <div className="flex flex-col gap-1 md:col-span-2">
              <label className="font-semibold">رسالة <span className="text-gray-400">(اختياري)</span></label>
              <input className="input" placeholder="رسالة (اختياري)" value={message} onChange={e => setMessage(e.target.value)} />
            </div>
          </div>
          <div className="text-center mt-6">
            <button type="submit" className="bg-primary-500 text-white px-8 py-2 rounded-lg font-bold hover:bg-primary-600 transition text-lg" disabled={loading}>
      ...
      <style jsx>{`
        .input {
          @apply border border-gray-300 rounded-lg px-4 py-2 w-full focus:outline-none focus:ring-2 focus:ring-primary-300 transition bg-white;
        }
      `}</style>
    </form>
  );
